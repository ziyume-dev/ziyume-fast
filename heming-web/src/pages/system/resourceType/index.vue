...
import { resourceTypePage } from '~/api/system/resourceType'
...
const handleSearch = () => {
  resourceTypePage(data.queryParam).then(res => {
...
<template>
  <n-card>
    <!-- 页头 -->
    <n-page-header subtitle="资源类别管理">
      <n-grid :x-gap="12" :y-gap="8" cols="1 400:2 600:4">
        <n-gi>
          资源类别名称
          <n-input v-model:value="data.queryParam.name" type="text" placeholder="请输入资源类别名称" clearable />
        </n-gi>
        <n-gi>
          资源类别编码
          <n-input v-model:value="data.queryParam.code" type="text" placeholder="请输入资源类别编码" clearable />
        </n-gi>
      </n-grid>
      <template #extra>
        <n-space>
