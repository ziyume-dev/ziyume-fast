...
const useUserPage = async (role: string) => {
  ...
  dataList.value = []
  const json = await nuxtApp.$api.post('/@fast-api/user/pageList', {
    json: data.queryParam,
  }).json();
  if (json.code === 200) {
...
const handleUserDelete = async () => {
  // const json = await nuxtApp.$api.delete(`/@fast-api/user/delete/${defaultUser.value.id}`).json();
  // if (json.code === 200) {
